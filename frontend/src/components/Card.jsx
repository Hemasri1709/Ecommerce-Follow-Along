...
const Card = ({ product, onDelete }) => {
    ...
    return (
      <div className="bg-black bg-opacity-50 shadow-lg rounded-lg p-4 hover:shadow-xl transition-shadow">
        <Link to={`/product/${product._id}`}>
          <img src={imageUrl} alt={product.name} className="w-full h-40 object-cover rounded-md" />
          <h3 className="text-xl font-semibold mt-2 text-white">{product.name}</h3>
